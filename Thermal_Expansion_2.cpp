...
// dom.AddBoxLength(1 ,Vec3_t ( -H/2.0, -H/2.0 , -H/2.0 ), 
    	...
    	for (size_t a=0; a<dom.Particles.Size(); a++)			
			...
			std::cout << "Inner Particle Number: "<< partcount << endl;
    	...
    	for (size_t a=0; a<dom.Particles.Size(); a++)
    	{
			...
			if (dom.Particles[a]->ID == 1)
				dom.Particles[a]->q_source = power / partcount * dom.Particles[a]->Density / dom.Particles[a]->Mass;					
    		
				// if ( x < (-H/2.0 + H/20.0) && y < (-H/2.0 + H/20.0) && z < (-H/2.0 + H/20.0) ) {
...
//    	dom.Solve(/*tf*/0.01,/*dt*/timestep,/*dtOut*/0.001,"test06",999);
		...
		dom.ThermalStructSolve(/*tf*/0.101,/*dt*/timestep,/*dtOut*/0.1,"test06",999);
