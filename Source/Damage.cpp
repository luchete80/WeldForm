      ...
      if ((rij - dam_r0_unif)/dam_r0_unif >= eps_max){
        dam_rf0[k][p] = rij;
			...
			//EVOLUTION
				...
				double m = -PP[0]->damage_model.sigma_max / (PP[0]->damage_model.delta_max - dam_rf0[k][p]); //SLOPE
				//mx+b = 0 (m*deltamax  + b = 0)
				double b = -m * PP[0]->damage_model.delta_max;
				//Calculate max transferable traction from linear cohesive law
				double sig_al_t = m * delta + b;
				if (sijrr > sig_al_t) {
					if (delta < PP[0]->damage_model.delta_max)
