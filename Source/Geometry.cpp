...
void Domain::AddCylSliceLength(int tag, double Rxy, double Lz, 
																				double r, double Density, double h, double alpha) {
...
void Domain::GenerateSPHMesh(const int &tag, NastranVolReader &nr,double Density, double hfac){
  ...
  std::vector<std::vector<short>>el_nod;
  for (int i=0;i<nr.node_count;i++){el_nod.push_back(std::vector<short>(50));}
