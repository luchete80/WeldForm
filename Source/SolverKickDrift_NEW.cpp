...
inline void Domain::SolveDiffUpdateKickDrift (double tf, double dt, double dtOut, char const * TheFileKey, size_t maxidx) {
  ...
  cout << "Calc initial acceleration "<<endl;
  ...
  int imax = 0;
		for (int i=0; i<Particles.Size(); i++){
			if (norm(Particles[i]->a) > max){
				max= norm (Particles[i]->a);
				imax=i;
			}
		}
    cout << "Max accel norm: "<< max <<" in particle "<<imax<<endl;
...
///////////////////////////////////////////////////    
    ...
    double factor = 1.;
    //////// THIS IS NEW, WITH PREV ACCEL CALCULATE VELOCITY
    clock_beg = clock();
    #pragma omp parallel for schedule (static) num_threads(Nproc)
    for (int i=0; i<Particles.Size(); i++){
      Particles[i]->v += prev_acc[i]*deltat/2.*factor;
    ...
    #pragma omp parallel for schedule (static) num_threads(Nproc)
    for (int i=0; i<Particles.Size(); i++){
      Particles[i]->v += Particles[i]->a*deltat/2.*factor; //Update with new acceleration
      //Particles[i]->LimitVel();
    }
    MoveGhost();
    GeneralAfter(*this);
