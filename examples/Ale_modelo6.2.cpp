...
void UserAcc(SPH::Domain & dom)
{
	...
	if ( dom.getTime() < T_WELD ) {
		...
		for (size_t i=0; i<dom.Particles.Size(); i++){
			...
			if (y <= (ARC_RADIUS +ARC_RADIUS/20 )&& y >= -ARC_RADIUS && z == THICKNESS/2. -rw + WALL_HEIGHT){
				// cout << "z "<< dom.Particles[i]->x(2)<<endl;
	...
	rw	= dxw/2.;
		...
		if (WALL_HEIGHT == 0.0){
     	dom.AddBoxLength(1 ,Vec3_t ( -L/2.0 , 0.0 , -T/2.0 ), 
			...
			dom.Add3DCubicBoxParticles(1, Vec3_t(-WELD_LENGTH/2.0,0,T/2.0),
										WELD_LENGTH +dxw, ARC_RADIUS + dxw,  WALL_HEIGHT,  
										rw, rho, h);
    	...
    	for (size_t a=0; a<dom.Particles.Size(); a++)
    	{
    		...
    		if ( z == -T/2.+ r || 
				( (z - (T/2.0 - r))<1e-5 && y > ARC_RADIUS + r ) 
				|| abs(y - W) < dx/2. 
				|| x == -L/2. + r || abs(x - L/2.)<dx/2. 
				|| ( abs(y - ARC_RADIUS) < dx/2. && z > T/2.0 && z < T/2 -r + WALL_HEIGHT)
				|| ( (z - (T/2.0 - r))<1e-5 && abs(x)> WELD_LENGTH/2. ) 
